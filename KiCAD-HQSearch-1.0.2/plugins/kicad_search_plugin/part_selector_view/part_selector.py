...
class PartSelectorframe( wx.Frame ):
    def __init__(self, parent=None):
        ...
        self.throttle_interval = 0.6  # 设置时间间隔，单位为秒
    ...
    def search_api_request(self, url, data):
        ...
        if not res_datas:
            ...
            return
        self.total_num = response.json().get("total", -1)
        if self.total_num == -1:
            self.total_num = 10
        if self.total_num == 0:
            self.current_page = 0
        ...
        for item in res_datas:
            if item.get("queryPartVO", {}).get("part", {}):
    ...
    def populate_part_list(self):
        ...
        parameters = ["mpn", "manufacturer", "package", "category", "sku", "price"]
        body = []
        ...
        for idx, part_info in enumerate(self.search_part_list, start=1):
            ...
            for data in res_datas:
                if data.get("mpn") == mpn and data.get("vendor") == "hqself":
                    ...
                    if _sku != " ":
                        # ，类似（https://item.hqchip.com/2500217622.html），
                        sku = "https://item.hqchip.com/"+_sku+ ".html"
                    else:
                        sku = _sku
                    # sku = data.get("sku", " ") 
    ...
    def on_right_down(self, e):
        conMenu = wx.Menu()
        selcet_part = wx.MenuItem(conMenu, ID_SELECT_PART, _("Jump to the product page") )
